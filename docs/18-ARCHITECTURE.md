...
- `src/App.tsx` — full-width page shell (no `max-w-3xl` cap; prose blocks are
  ...
  (Generate | Chat | Settings, `useState`, no router) + `<Toaster theme>`.
  Owns the active tab AND the one-shot `ChatAttachRequest` the gallery's
  "Chat with this image" raises (`onChat`), so the lightbox can put the owner
  on the Chat tab with that image staged (docs/17 row 17).
- `src/features/generate/GenerateArea.tsx` — the Create | Refine tablist + the
  ONE gallery both modes write to; owns the selected refinement source and
  forwards the gallery's `onChat` (the request itself is `App`'s, docs/17 row 17).
...
| Chat UI: conversation list (newest first) + turns (user one side, assistant text + images the other, per-turn and total cost) + composer disabled-with-reason (no key / no refinement pick, NO image-model fallback / model not `canRefineViaChat` / empty draft) + Cancel via AbortSignal + auto-scroll. Its ONE staged-attachment state (`attached`) is fed by BOTH the composer's file control AND a `ChatAttachRequest` handed over by `App` (docs/17 row 17), which it CONSUMES by reporting the nonce back — so the gallery path can never become a second attachment mechanism | `src/features/chat/ChatArea.tsx`, `src/features/chat/useChat.ts`, `src/features/chat/attachRequest.ts` | `tests/features/chat-panel.test.tsx`, `tests/features/chat-attach.test.tsx`, `tests/architecture/lightbox-chat-attach.test.ts` (`useState<StoredImage[]>` in exactly one file) | — |
| Gallery (newest-first multi-column grid, lightbox: prompt/model/cost/date/size, Download with MIME extension, Delete, "Refine this" → the refine panel, "Chat with this image" (only when the host passes `onChat`) → the Chat tab with that image staged, docs/17 row 17) + `useImageUrl` (object URL revoked on unmount). The lightbox is the FULL viewport as a flex column: the image area is `min-h-0 flex-1` and the image `max-h-full max-w-full object-contain`, with the metadata/buttons in a compact scrollable strip below (no `max-h-[70vh]` cap) | `src/features/gallery/` | `tests/features/generate-panel.test.tsx`, `tests/features/refine-panel.test.tsx`, `tests/features/chat-attach.test.tsx`, `tests/architecture/lightbox-chat-attach.test.ts` | Campaigner `src/features/images/use-image-url.ts` |
