...
- `src/components/styles.ts`, `src/components/ui.tsx` — the design vocabulary
  (button recipe, focus ring, `EmptyState`, `Segmented`); `src/lib/media.ts` —
  the ONE responsive breakpoint query.
...
| Gallery (newest-first grid, lightbox: prompt/model/cost/date/size, Download with MIME extension, Delete, "Refine this" → the refine panel, "Chat with this image" (only when the host passes `onChat`) → the Chat tab with that image staged, docs/17 row 18) + `useImageUrl` (object URL revoked on unmount). The lightbox is the FULL viewport as a flex column: the image area is `min-h-0 flex-1` and the image `max-h-full max-w-full object-contain`, with the metadata/buttons in a compact scrollable strip below (no `max-h-[70vh]` cap). Art-forward (docs/17 row 20): the grid is the hero surface, edge-to-edge, and the TILE captions carry the PROMPT only — the model id lives in the lightbox, once | `src/features/gallery/` | `tests/features/generate-panel.test.tsx`, `tests/features/refine-panel.test.tsx`, `tests/features/chat-attach.test.tsx`, `tests/architecture/lightbox-chat-attach.test.ts` | Campaigner `src/features/images/use-image-url.ts` |
