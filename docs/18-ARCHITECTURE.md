...
- `src/App.tsx` — app shell (no `max-w-3xl` cap; prose blocks are bounded
  ...
  the owner on the Chat tab with that image staged (docs/17 row 17).
...
| Reference prep: stored image OR uploaded file → ONE `data:` URL for an image part, long edge capped at `REFERENCE_MAX_EDGE_PX` = 1024 (aspect preserved, never upscaled), decode failure THROWS. Encoding a resized image is canvas-API-specific: `OffscreenCanvas` (preferred) has NO `toDataURL` and encodes ASYNCHRONOUSLY via `await convertToBlob({type})` → `Blob` → the shared `bytesToDataUrl`; only the detached-`<canvas>` fallback uses `toDataURL` (the live `r.canvas.toDataURL is not a function` bug, docs/17 row 14). An empty blob / the bare `"data:,"` sentinel and a zero-sized decode all THROW. Serves BOTH the Images API `input_references` path AND the chat path's history replay (docs/17 row 12) — the chat must not re-implement the downscale | `src/features/refine/reference.ts` | `tests/architecture/reference-encode.test.ts`, `tests/features/refine-panel.test.tsx`, `tests/features/chat-panel.test.tsx` | — |
...
| Chat UI: conversation list (newest first) + turns (user one side, assistant text + images the other, per-turn and total cost) + composer disabled-with-reason (no key / no refinement pick, NO image-model fallback / model not `canRefineViaChat` / empty draft) + Cancel via AbortSignal + auto-scroll. Its ONE staged-attachment state is the `useStagedAttachment` hook, fed by BOTH the composer's file control AND a `ChatAttachRequest` handed over by `App` (docs/17 row 17), which the hook CONSUMES by reporting the nonce back — so the gallery path can never become a second attachment mechanism. The list is the `conversation-list` seam's rail/sheet (docs/17 row 19) | `src/features/chat/ChatArea.tsx`, `src/features/chat/useChat.ts`, `src/features/chat/useStagedAttachment.ts`, `src/features/chat/attachRequest.ts` | `tests/features/chat-panel.test.tsx`, `tests/features/chat-attach.test.tsx`, `tests/architecture/lightbox-chat-attach.test.ts` (`useState<StoredImage[]>` in exactly one file) | — |
| Gallery (newest-first grid, lightbox: prompt/model/cost/date/size, Download with MIME extension, Delete, "Refine this" → the refine panel, "Chat with this image" (only when the host passes `onChat`) → the Chat tab with that image staged, docs/17 row 17) + `useImageUrl` (object URL revoked on unmount). The lightbox is the FULL viewport as a flex column: the image area is `min-h-0 flex-1` and the image `max-h-full max-w-full object-contain`, with the metadata/buttons in a compact scrollable strip below (no `max-h-[70vh]` cap). Art-forward (docs/17 row 19): the grid is the hero surface, edge-to-edge, and the TILE captions carry the PROMPT only — the model id lives in the lightbox, once | `src/features/gallery/` | `tests/features/generate-panel.test.tsx`, `tests/features/refine-panel.test.tsx`, `tests/features/chat-attach.test.tsx`, `tests/architecture/lightbox-chat-attach.test.ts` | Campaigner `src/features/images/use-image-url.ts` |
| Theme (read/apply/persist/watch): `getTheme`/`setTheme`/`toggleTheme`/`useTheme`, the `dark` class on `<html>`, localStorage `imager.theme` as the ONE source of truth, default DARK when nothing is stored. The flash-free half is the inline script in `index.html` (the ONLY inline script), pinned to agree with the seam. The CSS half owns the ROLE tokens (accent = indigo/violet, docs/17 row 19), the type scale, and the Inter stack (`@fontsource-variable/inter`, self-hosted — no external font request) | `src/lib/theme.ts`, `src/components/ThemeToggle.tsx`, `src/index.css` (`@custom-variant dark`, role tokens, type scale, control seams), `index.html` | `tests/features/theme.test.tsx`, `tests/architecture/theme-startup.test.tsx`, `tests/architecture/design-system.test.ts` (contrast floors) | — |
